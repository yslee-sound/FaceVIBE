...
object SaveUtils {
    ...
    fun saveBitmapToGallery(context: Context, bitmap: Bitmap, displayName: String = "FaceVIBE_${System.currentTimeMillis()}.png"): String {
        return if (Build.VERSION.SDK_INT >= 29) {
            val values = ContentValues().apply {
                put(MediaStore.Images.Media.DISPLAY_NAME, displayName)
                put(MediaStore.Images.Media.MIME_TYPE, "image/png")
                put(MediaStore.Images.Media.RELATIVE_PATH, Environment.DIRECTORY_PICTURES + "/FaceVIBE")
            ...
            val targetDir = File(dir, "FaceVIBE").apply { if (!exists()) mkdirs() }
